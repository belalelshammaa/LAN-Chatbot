...
int main(void) {
  ...
  printf("EL SERVER ZAY EL FOL WEH LISTENING 3ALA PORT %s AND WAITING FOR "
         "CLIENTS... \n",
         DEFAULT_PORT);
  ...
  while (1) {
    ...
    for (int i = 0; i < clients.len; i++) {
      struct Client *c =
          (struct Client *)((char *)clients.b + (i * clients.element_size));
    ...
    if (FD_ISSET(server_socket, &read_fds)) {
      ...
      if (new_socket < 0) {
        ...
        struct DataPacket first_packet;
        recv(new_socket, &first_packet, sizeof(struct DataPacket), 0);
        new_client.socket = new_socket;

        first_packet.payload[PAYLOAD_SIZE - 1] = '\0';
        printf("name i got: %s\n", first_packet.payload);
        sprintf(new_client.uuid, "%s%d", first_packet.payload, clients.len + 1);
        strcpy(new_client.name, first_packet.payload);
    ...
    for (int i = 0; i < clients.len; i++) {
      struct Client *c =
          // why not cast to Client * right away and do + i
          (struct Client *)((char *)clients.b + (i * clients.element_size));
      int current_client_socket = c->socket;
